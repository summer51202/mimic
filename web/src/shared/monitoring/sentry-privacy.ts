...
const STATIC_ROUTES = new Set([
  "/health", "/health/live", "/health/ready", "/api/v1/health", "/api/v1/health/live",
  "/api/v1/health/ready", "/api/health/live", "/api/health/ready",
]);
...
function isSafeRoute(value: unknown): value is string {
  if (typeof value !== "string" || value.length > 180 || !value.startsWith("/") || /[%?@#\\]/.test(value) || value.includes("..")) {
    return false;
  }

  if (STATIC_ROUTES.has(value)) return true;
  const segments = value.split("/").slice(1);
  return segments.some((segment) => /^:[A-Za-z][A-Za-z0-9]{0,31}$/.test(segment)) &&
    segments.every((segment) => /^(?::[A-Za-z][A-Za-z0-9]{0,31}|[a-z][a-z0-9-]{0,31})$/.test(segment));
}

function isSafeDnsHostname(value: string): boolean {
  return value.length <= 253 && value !== "localhost" && value.includes(".") &&
...
function sanitizeRequest(value: unknown): RecordValue | undefined {
  ...
  if (!method || !SAFE_METHODS.has(method) || !["http:", "https:"].includes(url.protocol) || url.username || url.password || !isSafeDnsHostname(url.hostname)) {
    return undefined;
  }
...
export function sanitizeSentryEvent(event: unknown): RecordValue {
  ...
  const environment = ["development", "test", "staging", "production"].includes(source.environment as string) ? source.environment : undefined;
